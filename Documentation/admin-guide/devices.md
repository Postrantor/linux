---
tip: translate by baidu@2024-01-30 21:33:11
---
---
...
> 此列表是Linux设备列表，是Linux操作系统的已分配设备编号和“/dev/”目录节点的官方注册表。


The version of this document at lanana.org is no longer maintained. This version in the mainline Linux kernel is the master document. Updates shall be sent as patches to the kernel maintainers (see the `Documentation/process/submitting-patches.rst <submittingpatches>`{.interpreted-text role="ref"} document). Specifically explore the sections titled \"CHAR and MISC DRIVERS\", and \"BLOCK LAYER\" in the MAINTAINERS file to find the right maintainers to involve for character and block devices.

> lanana.org上不再保留本文件的版本。主流Linux内核中的这个版本是主文档。更新应作为补丁发送给内核维护人员（请参阅`Documentation/process/submitting-patches.rst<submittingpatches>`{.depredicted text role=“ref”}文档）。具体探索MAINTAINERS文件中标题为“CHAR和MISC DRIVERS”和“BLOCK LAYER”的部分，以找到适合角色和块设备的维护人员。
...
> 本文档通过引用包含在文件系统层次结构标准（FHS）中。FHS可从<https://www.pathname.com/fhs/>.
...
> 此文档属于公共领域。然而，作者要求，未经作者许可，不得分发语义更改的版本，前提是可以在不付出不合理努力的情况下联系作者。
...
> Linux现在广泛支持设备编号的动态分配，并可以使用“sysfs”和“udev”（“systemd”）来处理命名需求。在串行和引导设备区域仍有一些例外。在询问设备编号之前，请确保您确实需要一个。
...
> 要分配一个主要号码，或在适用的情况下分配一个次要号码（如busmices），请提交一个补丁并发送给上述作者。
...
> 将设备的说明*保持与此列表*相同的格式。原因是，这是我们找到的唯一方法，可以确保我们拥有发布您的设备并避免冲突的所有必要信息。
...
> 最后，有时我们不得不扮演“命名空间警察”。请不要被冒犯。我们经常收到“/dev”名称的提交，这些名称势必会在以后引发冲突。我们正在努力避免陷入这样一种境地，即我们将不得不承受一种不兼容的向前变化。因此，在**以任何方式公开您的设备名称和编号之前，请咨询我们**，至少到了很难更改的程度。
...
> 本节详细介绍了/dev/目录中应该存在或可能存在的其他条目。符号链接最好使用与此处所示相同的形式（绝对或相对）。根据首选的链接类型，链接被分类为“硬链接”或“符号链接”；如果可能的话，应该使用所指示的链接类型。
...
> 以下链接可以在本地建立，以符合系统的配置。这只是对现有做法的总结，并不构成建议。但是，如果它们存在，则应具有以下用途。
...
> 对于SCSI设备，“/dev/tape”和“/dev/cdrom”应指向*cooked*设备（分别为“/dev/st*”和“/dev/sr*”），而“/dev/cdwriter”和/dev/scanner应指向适当的通用SCSI设备（/dev/sg\*）。
...
> 以下名称是为在/dev下安装特殊文件系统而保留的。这些特殊的文件系统提供了标准设备节点无法提供的内核接口。
...
> 终端或TTY设备是一类特殊的字符设备。终端设备是可以充当会话的控制终端的任何设备；这包括虚拟控制台、串行端口和伪终端（PTY）。
...
> 所有终端设备共享一组共同的能力，称为线路规程；其中包括公共终端线路规程以及SLIP和PPP模式。
...
> 所有终端设备的名称相似；本节介绍了各种类型TTY的命名和使用。请注意，命名约定包括几个历史缺陷；其中一些是特定于Linux的，一些是从其他系统继承的，还有一些反映了Linux超出了借用的约定。


A hash mark (`#`) in a device name is used here to indicate a decimal number without leading zeroes.

> 设备名称中的散列标记（“#”）用于指示不带前导零的十进制数。
...
> 虚拟控制台是系统视频监视器上的全屏终端显示器。虚拟控制台名为“/dev/tty#”，编号从“/dev/tty1”开始`/dev/ty0`是当前的虚拟控制台`/“dev/ty0”是应该用于访问帧缓冲设备（“/dev/fb*”）不适用的那些体系结构上的系统视频卡的设备。不要将“/dev/console”用于此目的。
...
> 控制台设备“/dev/console”是应向其发送系统消息的设备，并且应允许在单用户模式下登录。从Linux 2.1.71开始，“/dev/console”由内核管理；对于以前的版本，它应该是指向“/dev/tty0”、特定虚拟控制台（如“/dev/tty1”）或串行端口主设备（“tty*”，而不是“cu*”）的符号链接，具体取决于系统的配置。
...
> 串行端口是RS-232串行端口和任何在硬件（如内部调制解调器）或软件（如ISDN驱动程序）中模拟一个串行端口的设备。在Linux下，每个串行端口都有两个设备名称，主设备或呼叫设备和备用设备或调用设备。每种设备都用不同的字母表示。对于任何字母X，设备的名称分别为“/dev/ttyX#”和“/dev/cux#”；由于历史原因，“/dev/ttyS#”和“/dev/ttyC#”对应于“/dev/cua#”和”/dev/cub#“。在未来，应该预计将使用多个字母；所有字母都是\“tty”设备的大写字母（例如“/dev/ttyDP#”）和\“cu”设备的小写字母（例如：“/dev/cudp#”）。
...
> 备用设备提供基于内核的排除和与主设备稍有不同的默认设置。它们的主要目的是允许将串行端口与程序一起使用，而对串行端口没有固有的或中断的支持。它们的使用已被弃用，并且可能会从未来版本的Linux中删除。
...
> 串行端口的仲裁是通过使用名为“/var/lock/LCK”的锁定文件来提供的。。ttyX#`。锁定文件的内容应该是锁定过程的PID，作为ASCII数字。
...
> 通常的做法是安装指向串行端口的链接，如/dev/modem。为了确保在存在这些链接的情况下正确锁定，建议软件追逐符号链接并锁定所有可能的名称；此外，建议将锁文件与相应的备用设备一起安装。为了避免死锁，建议按以下顺序获取锁，并按相反顺序释放：
...
> 在嵌套符号链接的情况下，应按照解析符号链接的顺序安装锁定文件。
...
> 在任何情况下，应用程序都不应在等待另一个锁释放时持有锁。此外，试图为相应的备用设备名称创建锁定文件的应用程序应考虑在不存在备用设备的非串行端口TTY上使用的可能性。
...
> 伪终端或PTY用于创建登录会话或向任意数据生成过程提供需要TTY线路规程的其他功能（包括SLIP或PPP功能）。每个PTY都有一个主端，名为“/dev/PTY[p-za-e][0-9a-f]”，还有一个从端，名“/dev/tty[p-zal-e][0-9a-f]”。内核通过允许每个主端只打开一次来仲裁PTY的使用。
...
> 一旦主设备侧被打开，相应的从设备就可以以与任何TTY设备相同的方式使用。主设备和从设备由内核连接，生成具有TTY功能的双向管道。
...
> 最新版本的Linux内核和GNU libc包含对用于PTY的System V/Unix98命名方案的支持，该方案将一个通用设备“/dev/ptmx”分配给所有主设备（打开它将自动为您提供一个以前未分配的PTY）和一个子目录“/dev/pts”，用于从属设备；从设备是用十进制整数命名的（在我们的记法中是“/dev/pts/#”）。这消除了耗尽命名空间的问题，并使内核能够根据需要使用“devpts”文件系统自动为从属设备创建设备节点。
