---
tip: translate by baidu@2024-01-30 21:35:04
---
---
...
The following is a consolidated list of the kernel parameters as implemented by the \_\_setup(), early_param(), core_param() and module_param() macros and sorted into English Dictionary order (defined as ignoring all punctuation and sorting digits before letters in a case insensitive manner), and with descriptions where known.

> 以下是由\\_setup（）、early_param（）、core_param（。


The kernel parses parameters from the kernel command line up to \"`--`\"; if it doesn\'t recognize a parameter and it doesn\'t contain a \'.\', the parameter gets passed to init: parameters with \'=\' go into init\'s environment, others are passed as command line arguments to init. Everything after \"`--`\" is passed as an argument to init.

> 内核从内核命令行解析参数，直到\“`--`\”；如果它不识别参数并且不包含\'.\'，参数被传递给init：带有\'=\'的参数进入init的环境，其他参数作为命令行参数传递给init。“`--`\”之后的所有内容都作为参数传递给init。


Module parameters can be specified in two ways: via the kernel command line with a module name prefix, or via modprobe, e.g.:

> 模块参数可以通过两种方式指定：通过带有模块名称前缀的内核命令行，或者通过modprobe，例如：
...
> 内核中内置的模块的参数需要在内核命令行中指定。modprobe在加载模块时会查看内核命令行（/proc/cmdline）并收集模块参数，因此内核命令行也可以用于可加载模块。
...
> 一些内核参数以CPU列表为值，例如isolcpus、nohz_full、irqaffinity、rcu_nocbs。此列表的格式为：
...
> 请注意，对于范围的特殊情况，可以将范围划分为大小相等的组，并且对于每个组，从该组的开始使用一些量：
...
> 值“N\”可用于表示系统上最后一个CPU的数值，即“foo_cpus=16-N\”相当于32核系统上的“16-31\”。
...
> 请记住，“N”是动态的，因此，如果系统更改导致位图宽度发生变化，例如CPU列表中的内核减少，则N和使用N的任何范围也将发生变化。在小型4核系统上使用相同的方法，“16-N\”变为“16-3\”，现在相同的引导输入将被标记为无效（开始\>结束）。
...
> 允许特殊情况的组名“all\”具有选择所有CPU的含义，因此“nohz_full=all\”相当于“nohz _full=0-N\”。


The semantics of \"N\" and \"all\" is supported on a level of bitmaps and holds for all users of bitmap_parselist().

> “N”和“all”的语义在位图级别上得到支持，适用于bitmap_parselist（）的所有用户。


This document may not be entirely up to date and comprehensive. The command \"modinfo -p \${modulename}\" shows a current list of all parameters of a loadable module. Loadable modules, after being loaded into the running kernel, also reveal their parameters in /sys/module/\${modulename}/parameters/. Some of these parameters may be changed at runtime by the command `echo -n ${value} > /sys/module/${modulename}/parameters/${parm}`.

> 本文件可能不完全是最新的和全面的。命令“modinfo-p\$｛modulename｝”显示可加载模块的所有参数的当前列表。可加载模块在加载到正在运行的内核中后，也会在/sys/module/\$｛modulename｝/parameters/中显示其参数。其中一些参数可以在运行时通过命令“echo-n$｛value｝>/sys/module/$｛modulename｝/parameters/$｛parm｝”进行更改。
...
> 以下列出的参数仅在启用了某些内核构建选项并且存在相应硬件的情况下有效。这份清单应按字母顺序排列。每个描述开头方括号中的文本说明了参数适用的限制：
...
> 用BOOT表示的参数实际上是由引导加载程序解释的，对内核没有直接意义。在没有极端需要或与\<Documentation/arch/x86/boot.rst\>协调的情况下，不要修改引导加载程序参数的语法。
...
> 还有一些特定于arch的内核参数没有在这里记录。例如，请参阅\<Documentation/arch/x86_64/boot options.rst\>。
...
> 请注意，下面列出的所有内核参数都是CASE SENSITIVE，并且任何参数名称上的尾随=表示该参数将作为环境变量输入，而不存在表示它将显示为内核参数，系统启动后运行的程序可通过/proc/cmdline读取该参数。
...
> 内核参数的数量不受限制，但完整命令行的长度（包括空格等的参数）限制为固定数量的字符。此限制取决于体系结构，介于256到4096个字符之间。它在文件中定义/包括/uapi/asm generic/setup.h作为COMMAND_LINE_SIZE。


Finally, the \[KMG\] suffix is commonly described after a number of kernel parameter values. These \'K\', \'M\', and \'G\' letters represent the \_[binary]() multipliers \'Kilo\', \'Mega\', and \'Giga\', equaling 2\^10, 2\^20, and 2\^30 bytes respectively. Such letter suffixes can also be entirely omitted:

> 最后，\[KMG\]后缀通常在一些内核参数值之后进行描述。这些“K”、“M”和“G”字母分别代表“Kilo”、“Mega”和“Giga”的[binary]（）乘法器，分别等于2-10、2/20和2/30字节。这样的字母后缀也可以完全省略：
