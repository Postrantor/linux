...
which indicates the CPU will still see idle time since 900 is \< 1024. The \_[actual]() util_avg will not be 900 though, but somewhere between 300 and 900. As long as there\'s idle time, p-\>util_avg updates will be off by a some margin, but not proportional to Fmax/Fmin.
...
which is higher than 1024 and indicates that the CPU has no idle time. When this happens, then the \_[actual]() util_avg will become:
