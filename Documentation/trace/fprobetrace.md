...
As you can see, the `sched_switch` trace-event shows _cooked_ parameters, on the other hand, the `sched_switch` tracepoint probe event shows _raw_ parameters. This means you can access any field values in the task structure pointed by the `prev` and `next` arguments.
